...
func (r *TrainingJobReconciler) syncAutogeneratedScale(job *kaiv1alpha1.TrainingJob) (bool, error) {
	scaleOuts := &kaiv1alpha1.ScaleOutList{}
	err := r.Client.List(context.Background(), scaleOuts, &client.ListOptions{LabelSelector: labels.SelectorFromSet(labels.Set(map[string]string{
		"auto-generated": "true",
		"job-id":         job.Name,
	}))})
	...
	scaleIns := &kaiv1alpha1.ScaleInList{}
	err = r.Client.List(context.Background(), scaleIns, &client.ListOptions{LabelSelector: labels.SelectorFromSet(labels.Set(map[string]string{
		"auto-generated": "true",
		"job-id":         job.Name,
	}))})
	if err != nil && !errors.IsNotFound(err) {
	...
	var availableScales []Scaler
	for _, scaleout := range scaleOuts.Items {
		if !filterAvailableScaler(&scaleout, job) {
			continue
		}
		availableScales = append(availableScales, scaleout.DeepCopy())
	}
	for _, scalein := range scaleOuts.Items {
		if !filterAvailableScaler(&scalein, job) {
			continue
		}
		availableScales = append(availableScales, scalein.DeepCopy())
	}
	if len(availableScales) > 1 {
		return true, fmt.Errorf("too many auto-generated scaleIn/scaleOut")
	}

	if len(availableScales) == 0 {
		pods, err := r.GetWorkerPods(job)
		if err != nil {
			return true, err
		}
		expectReplicas := int(*job.Spec.ETReplicaSpecs.Worker.Replicas)
		if len(pods) != expectReplicas {
			return true, r.scaleByJobReplica(job, expectReplicas, len(pods))
		}
	}

	if len(availableScales) == 1 {
		return true, r.updateCurrentScaler(job, availableScales[0])
	}
	return false, nil
}
...
func (r *TrainingJobReconciler) reconcileJobRunning(job *kaiv1alpha1.TrainingJob) error {
	...
	if hasAutoGenScale, err := r.syncAutogeneratedScale(job); hasAutoGenScale {
		return err
	}
...
func (r *TrainingJobReconciler) scaleByJobReplica(job *kaiv1alpha1.TrainingJob, expect, actual int) error {
	var newScaler Scaler
	if expect > actual {
		newScaler = &kaiv1alpha1.ScaleOut{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: job.Name + "-scaleout",
			...
			Spec: kaiv1alpha1.ScaleOutSpec{
				ToAdd: &kaiv1alpha1.ToAddSpec{
					Count: utilpointer.Int32Ptr(int32(expect - actual)),
				},
				Selector: kaiv1alpha1.Selector{
					Name: job.Name,
				},
			},
		}
	} else {
		newScaler = &kaiv1alpha1.ScaleIn{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: job.Name + "-scalein",
				Namespace:    job.Namespace,
				Labels: map[string]string{
					"auto-generated": "true",
					"job-id":         job.Name,
			...
			Spec: kaiv1alpha1.ScaleInSpec{
				ToDelete: &kaiv1alpha1.ToDeleteSpec{
					Count: actual - expect,
				},
				Selector: kaiv1alpha1.Selector{
					Name: job.Name,
	...
	err := r.Client.Create(context.TODO(), newScaler)
	if err != nil {
		return err
	}
	return r.updateCurrentScaler(job, newScaler)
}
